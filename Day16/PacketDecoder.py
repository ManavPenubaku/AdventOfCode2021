...
def evaluate_packet_1(type_id,prev,current):
    ...
    print(current)
    eval = 0
    if(type_id == 0):eval = prev+current
...
def evaluate_packet_2(type_id,prev,current):
    print(type_id)
    if(type_id == 5):eval = 1 if prev > current else 0
    ...
    while True:
        ...
        if(id == 2):
            if(packet_count == check_finish):
                ...
                cur_pos = min(cur_pos,len(binary_input)-1)
                break
            elif(cur_pos+3 > len(binary_input) and check_finish > len(binary_input)):
        ...
        if(operator_id < 4):
            ...
            if(compare_flag == 1):
                packet_value = evaluate_packet_2(operator_id, prev_value, literal_value)
            else:
                prev_value = literal_value
                compare_flag = 1  
